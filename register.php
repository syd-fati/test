...
if (isset($_POST['register'])) {
    ...
    $ShippingInfo = trim($_POST['ShippingInfo']);
    $Username = trim($_POST['Username']);
    $Email = trim($_POST['Email']);
    $PhoneNo = trim($_POST['PhoneNo']);
    $Password = trim($_POST['Password']);
    $customer1 = trim($_POST['customer1']);
    ...
    if (count($errors) <= 0) {
        try {
            ...
            $sql->execute([$Username, $hashedPassword, $Email, $customer1]);
            ...
            $sql = $db->prepare("INSERT INTO customer1 (Fname, Lname, PhoneNo, Email, Address, DOB, CreditCardInfo, ShippingInfo, UserID)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $sql->execute([$Fname, $Lname, $PhoneNo, $Email, $Address, $DOB, $CreditCardInfo, $ShippingInfo, $idused]);
echo"done";
...
First Name: <input type="text" name="FName" required><br><br>
        ...
        Credit Card Information: <input type="number" name="CreditCardInfo" required><br><br>
        Shipping Information: <input type="text" name="ShippingInfo" required><br><br>
